...
class Pilipinas extends CI_Controller {
		...
		public function details(){
			...
			//kunin ang laman para contribution approve
			$contribute_list = $this->pinasmodel->contribute_getlist($placeid);
			$details_contribute = null;
			$list = null;
			if($contribute_list != 'false'){
				foreach ($contribute_list as $n){
					$list = array(
					...
					$details_contribute[] = $list;
				
				}
				
				$data['contribute'] = $details_contribute;
			}
			else {
				$data['contribute'] = 'none';
			}
		...
		public function redirect_details($details_complete){
			$data['title'] = "Pili-pinas";
			$data['contents'] = "Display bootstrap contents";
		...
		public function spam(){
			...
			$data['contents'] = "Display bootstrap contents";
				...
				$oldreports = $row['reports'];
				$newreports = $oldreports + 1;
			...
			$this->pinasmodel->update_reviewrow($updated_row, $review_id);
		...
		public function home_admin(){
			...
			$count2 = $this->pinasmodel->count_pending_contribution();
				$data['count2'] = $count2;
			$data['count'] = $count;
			$data['visits']= $countv;
		...
		public function contribution(){
			...
			$config['upload_path'] = './img/uploaded/'; //The path where the image will be save
			$config['allowed_types'] = 'jpeg|jpg|png'; //Images extensions accepted
			$config['overwrite'] = TRUE; //If exists an image with the same name it will overwrite. Set to  false if don't want to overwrite
			$this->load->library('upload', $config); //Load the upload CI library
			...
			$tcontribution_id = $this->pinasmodel->latest_pendingcontributionid();
			$new_contribution_id = $tcontribution_id +1;
			
			$new_contribution = array (
				'tcontribution_id' => $new_contribution_id,
				'name' => $name,
				'image' => $upload_image1,
				'caption' => $caption,
				'place_id' => $place_id
			...
			//ilagay sa pending contribution table
			$contributiontablename = "pending_contribution";      // name of table
			$new_review = $this->pinasmodel->review_insert($contributiontablename, $new_contribution); // send to model to insrt
			
			//get the review list
			$review_list = $this->pinasmodel->review_getlist($place_id);
			$details_review = null;
			...
			//kunin laan ng place
			 $new_rating = $this->pinasmodel->place_page($place_id);
			 $details_complete = null;
			...
			$data['details_page'] = $details_complete;
			//kunin ang laman para contribution approve
			$contribute_list = $this->pinasmodel->contribute_getlist($place_id);
			$details_contribute = null;
			$list = null;
			if($contribute_list != 'false'){
				foreach ($contribute_list as $n){
		...
		//open lang nung contributoion pending
		public function contributions_1_views(){
			$data['title'] = "Pili-pinas";
			$data['contents'] = "Display bootstrap contents";
			//kunin lahat ng pending contributions 
			$contribution = $this->pinasmodel->pendingcontributions();
			
			
			
			
			if($contribution != 'false')	{
				foreach($contribution as $n){
				$name = '';
				$name = $this->pinasmodel->placename($n['place_id']);
				
				
					$id = array(
					
						'tcontribution_id'   => $n['tcontribution_id'],
						'name'        => $n['name'],
						'caption'		 => $n['caption'],
						'image'		 => $n['image'],
						'place_id'   	 => $n['place_id'],
			...
			//kunin ung place name
		
			
				$data['contributions'] = $contributions; 
				
				$count = $this->pinasmodel->count_pending_contribution();
				$data['count'] = $count;	
			}
			
			//pag walang laman
			else {
				$data['contributions'] = 'No pending contributions';
				$data['count'] = '0';
				
			}
			
			$this->load->view('include/header', $data);
			$this->load->view('webview/admin_contribution', $data);
		}
		public function contributions_2_views(){
			$data['title'] = "Pili-pinas";
			$data['contents'] = "Display bootstrap contents";
			
			//kunin lahat ng approved contributions 
			$contribution = $this->pinasmodel->approvedcontributions();
			...
			//kunin ung place name
				...
				$count = $this->pinasmodel->count_approved_contribution();
				$data['count'] = $count;	
			}
			
			//pag walang laman
			else {
				$data['contributions'] = 'No approved contributions';
				$data['count'] = '0';
				
			}
			
			$this->load->view('include/header', $data);
			$this->load->view('webview/admin_contribution_2', $data);
		}
		
		public function delete_pending_contribution(){
			$data['title'] = "Pili-pinas";
			$data['contents'] = "Display bootstrap contents";
			
			$tcontribution_id = $_POST['tcontribution_id'];
			$table = "pending_contribution";
			$this->pinasmodel->delete_pending_contribution($table, $tcontribution_id);
		...
		public function delete_approved_contribution(){
			$data['title'] = "Pili-pinas";
			$data['contents'] = "Display bootstrap contents";
			...
			$this->pinasmodel->delete_approved_contribution_model($table, $contribution_id);

			
			$this->contributions_2_views();
		}
		
		// pending to approve contribution
		public function add_pending_contribution(){
			$data['title'] = "Pili-pinas";
			$data['contents'] = "Display bootstrap contents";
			$tcontribution_id = $_POST['tcontribution_id'];
			$table_tcontribution = 'pending_contribution';
			
			//kunin ang laman ni pending contribution row
			$tcontribution_row = $this->pinasmodel->getpendingcontributionrow($table_tcontribution, $tcontribution_id);
			$row = array(
					'tcontribution_id'   => $tcontribution_row[0]['tcontribution_id'],
					'name'    			=> $tcontribution_row[0]['name'],
					'caption'	  		=> $tcontribution_row[0]['caption'],
					'image'	  		=> $tcontribution_row[0]['image'],
					'place_id'    		=> $tcontribution_row[0]['place_id']
					
				);
				
			//kunin ang latest approved contribution row
			$contribution_id = $this->pinasmodel->latest_approvedcontributionid();
			
			
			$new_contributionid = $contribution_id + 1;
			
			$new_contribution = array (
					'contribution_id'   => $new_contributionid,	
					'name'    			=> $row['name'],
					'caption'	  		=> $row['caption'],
					'image'	  			=> $row['image'],
					'place_id'    		=> $row['place_id']
			...
			//ipasa na sa approve contribution
			$tablename = "contribution"; 
			$this->pinasmodel->moveto_approved_contribution($tablename, $new_contribution);
				
				
			//i-delete na sa pending_review
			$this->pinasmodel->delete_pending_contribution($tablename, $tcontribution_id);
			
			//kunin ulit ung pending list
			$contribution = $this->pinasmodel->pendingcontributions();
			if($contribution != 'false')	{
				foreach($contribution as $n){
				$name = '';
		...
		public function deleteplace(){
			...
			$data['contents'] = "Display bootstrap contents";
				...
				$place_id = $_GET['place_id'];
				$table = "place";
				$this->pinasmodel->deleteplace($table, $place_id);
				$categ = 0;
			$top = $this->pinasmodel->travelsort($categ);
		...
		//history page
		public function historyselect(){
			$data['title'] = "Pili-pinas";
			$data['contents'] = "Display bootstrap contents";
			
			$history_id = $_GET['history_id'];
			//kuhanin ang details ng lugar na yun
			$result = $this ->pinasmodel -> historydetails($history_id);
			$details_complete = null;
			$id = array(
			...
			$this->load->view('include/header', $data);
			$this->load->view('webview/history_page', $data);
